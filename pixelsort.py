...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="Dont show image after processing",
)
parser.add_argument(
    "--saturation",
    dest="saturation",
...
parser.add_argument(
    "--sort-start",
    dest="sort_to_start",
    default=SORT_TO_START,
    action="store_true",
    help="start sort from start of line",
)
parser.add_argument(
    "--sort-end",
    dest="sort_to_end",
    default=SORT_TO_END,
    action="store_true",
    help="sort until the end of the line",
)
parser.add_argument(
    "--noise",
    dest="random_sort",
...
parser.add_argument(
    ...
    default=WIDTH,
    type=int,
    help="how large to rescale the image",
)
parser.add_argument(
    "--max-chunk",
    dest="max_chunk",
    default=MAX_CHUNK,
    type=int,
    help="how many chunks to use with the delay effect",
)
parser.add_argument(
    "--num-chunks",
    dest="num_chunks",
    default=NUM_CHUNK,
    type=int,
    help="if sortable area is too large, break into this many chunks",
)
parser.add_argument(
    "--white-threshold",
    dest="black_threshold",
    type=int,
    default=BLACK_THRESHOLD,
    help='threshold for a pixel to be considered "black"',
)
parser.add_argument(
    "--black-threshold",
    ...
    help='threshold for a pixel to be considered "white"',
)

# animation args
parser.add_argument(
    "--animate", dest="animate", default=ANIMATE, action="store_true", help="animate"
)
parser.add_argument(
    "--jitter",
    dest="jitter",
    default=JITTER,
    type=int,
    help="how much shoudl animation jitter?",
)
parser.add_argument(
    "--smooth",
    dest="smooth",
...
parser.add_argument(
    ...
    default=10,
    type=int,
    help="how many distortions to run",
)
parser.add_argument(
    "--fps",
    dest="iterations",
    default=10,
    type=int,
    help="how many frames per distortion",
)
...
def sort_from(im, pix, big_start, big_end):
    ...
    if args.num_chunks:
        if big_end - big_start > args.max_chunk:
            ...
            for i in range(delta / chunk_size):
                sort_from(
                    im,
                    pix,
                    big_start + (i * chunk_size),
                    big_start + ((i + 1) * chunk_size),
                )
    ...
    if args.reverse:
        to_sort.sort(
            key=lambda v: -v[0][idx]
            + ((args.random_sort or 0) and random.randint(-10, 10))
        )
    else:
        to_sort.sort(
            key=lambda v: v[0][idx]
            ...
            for sort in SORTS:
                ...
                for x in range(start, end):
                    if distortions - j > 1 and args.smooth:
                        speed_adj = (hash(str(x)) % (distortions - j)) - (
                            distortions - j
                        ) / 2
                        x += speed_adj
